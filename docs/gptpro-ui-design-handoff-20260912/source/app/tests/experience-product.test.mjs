...
test('a concrete one-question clarification does not need a forced pair of scenarios',async()=>{
  let calls=0;
  const result=await runDesignDialogue({reviewProvider:approveReview,store:createSceneStore(createReferenceHome()),input:'厨房很挤',provider:async()=>{
    calls++;
...
test('every planner turn keeps DeepSeek thinking enabled with a bounded low-effort output budget',async()=>{
  let observed;
  const result=await runDesignDialogue({reviewProvider:approveReview,store:createSceneStore(createReferenceHome()),input:'客厅让我不想待',provider:async context=>{
    observed={reasoningEffort:context.reasoningEffort,maxTokens:context.maxTokens};
    return providerResult({action:'clarify',question:'你更想先改变坐下休息，还是朋友来时的感觉？'});
  }});
  assert.deepEqual(observed,{reasoningEffort:'low',maxTokens:8192});assert.equal(result.trace.mode,'clarify');
});
...
test('multi-kind rules sharing one quote remain valid across a subsequent model clarification',async()=>{
  ...
  const result=await runDesignDialogue({reviewProvider:approveReview,store,input:'沙发可以偏浅一点吗',requirements,provider:async()=>providerResult({action:'clarify',question:'你想要偏白的亚麻还是偏米的暖色？'})});
...
test('a multi-question clarification is repaired by the actual provider loop before user exposure',async()=>{
  ...
  const result=await runDesignDialogue({reviewProvider:approveReview,store,input:'想让朋友来坐得更舒服，工作桌要保留',provider:async context=>{
...
test('a named-room discomfort cannot fall back to a generic scene question',async()=>{
  let attempts=0;const store=createSceneStore(createReferenceHome());
  const result=await runDesignDialogue({reviewProvider:approveReview,store,input:'感觉厨房太挤了',selectedObjectId:'surface-floor-living-dining',activeRoomId:'room-living-dining',provider:async context=>{
...
test('open dialogue observes tool output and repairs invalid atomic candidate without leaking first action',async()=>{
  ...
  const result=await runDesignDialogue({reviewProvider:approveReview,store,input:'我想暖一点',provider:async context=>{
    n++;
...
test('unknown tool, fixed constraints, repeated observations and failed provider do not mutate scene',async()=>{
  ...
  const result=await runDesignDialogue({reviewProvider:approveReview,store,input:'清爽一点',requirements,provider:async()=>providerResult({action:'preview',toolCalls:[{tool:'delete_object',args:{objectId:'object-sofa'}}]})});
  ...
  await assert.rejects(runDesignDialogue({reviewProvider:approveReview,store,input:'暖一点',provider:async()=>{throw Error('DEEPSEEK_AUTH_FAILED');}}),/DEEPSEEK_AUTH_FAILED/);
});
...
test('actual experience HTTP caller returns preview/requirements and never silently falls back',async t=>{
  ...
  const server=createAppServer({experienceHandler:createExperienceRoutes({reviewProvider:approveReview,store,provider:async()=>providerResult({action:'clarify',question:'你最想改变的是坐下休息还是招待朋友时的感觉？'})})});
...
test('remote preview decisions survive reopen, block overlapping turns, and keep/save or discard atomically',async t=>{
  const directory=mkdtempSync(join(tmpdir(),'opai-preview-lifecycle-'));
  const store=createExperienceStore({directory});
  let discussionWrites=0;
  const family={listDiscussions:()=>[],createDiscussion:async()=>{discussionWrites++;return {id:'should-not-run'};}};
  const provider=async()=>providerResult({action:'preview',assistantReply:'先看沙发换成暖白色。',
    requirementsPatch:{confirmed:[{text:'沙发暖一点',quote:'沙发暖一点',kind:'preference',objectIds:['object-sofa']}]},
    toolCalls:[materialCall],reasons:[]});
  const server=createAppServer({experienceHandler:createExperienceRoutes({reviewProvider:approveReview,store,provider,family})});
  await new Promise(resolve=>server.listen(0,'127.0.0.1',resolve));
  t.after(async()=>{await new Promise(resolve=>server.close(resolve));rmSync(directory,{recursive:true,force:true});});
  const base=`http://127.0.0.1:${server.address().port}/api/experience/projects`;
  const create=()=>fetch(base,{method:'POST',body:'{}'}).then(response=>response.json());
  const post=(project,path,body)=>fetch(`${base}/${project.projectId}${path}`,{method:'POST',headers:{authorization:`Bearer ${project.accessToken}`,'content-type':'application/json'},body:JSON.stringify(body)});

  const discarded=await create();
  const firstResponse=await post(discarded,'/turn',{requestId:'preview:discard_turn',expectedRevision:0,input:'沙发暖一点',versionHistory:discarded.versionHistory});
  const first=await firstResponse.json();
  assert.equal(firstResponse.status,200);assert.equal(first.preview.status,'pending');assert.equal(first.revision,1);
  assert.match(first.preview.id,/^preview-[a-f0-9-]{36}$/);
  assert.equal(first.requirements.confirmed.length,1);
  assert.equal(store.read(discarded.projectId,discarded.accessToken).requirements.confirmed.length,0);
  const pendingDiscussion=await post(discarded,'/discussions',{requestId:'pending-discussion',versionId:'version-initial'});
  assert.equal(pendingDiscussion.status,409);assert.equal((await pendingDiscussion.json()).error,'PREVIEW_DECISION_REQUIRED');
  const blockedResponse=await post(discarded,'/turn',{requestId:'overlap-turn',expectedRevision:1,input:'再改一点',versionHistory:discarded.versionHistory});
  assert.equal(blockedResponse.status,409);assert.equal((await blockedResponse.json()).error,'PREVIEW_DECISION_REQUIRED');
  const discardResponse=await post(discarded,`/previews/${first.preview.id}/discard`,{requestId:'discard-decision',expectedRevision:1});
  const discardResult=await discardResponse.json();
  assert.equal(discardResponse.status,200);assert.equal(discardResult.revision,2);assert.equal(discardResult.requirements.confirmed.length,0);
  const discardedReopen=store.read(discarded.projectId,discarded.accessToken);
  assert.equal(discardedReopen.pendingPreview,null);assert.equal(discardedReopen.workingDraft,null);
  assert.equal(discardedReopen.requirements.confirmed.length,0);
  assert.equal(discardedReopen.versionHistory,discarded.versionHistory);

  const kept=await create();
  const preview=await (await post(kept,'/turn',{requestId:'preview-keep-turn',expectedRevision:0,input:'沙发暖一点',versionHistory:kept.versionHistory})).json();
  const keepResponse=await post(kept,`/previews/${preview.preview.id}/keep`,{requestId:'keep-decision',expectedRevision:1});
  const keepResult=await keepResponse.json();
  assert.equal(keepResponse.status,200);assert.equal(keepResult.revision,2);assert.equal(keepResult.requirements.confirmed.length,1);
  assert.equal(keepResult.conversationDelta[0].previewDecision.decision,'keep');
  const keptReopen=store.read(kept.projectId,kept.accessToken);
  assert.equal(keptReopen.pendingPreview,null);assert.equal(keptReopen.workingDraft.status,'kept');
  assert.equal(keptReopen.requirements.confirmed.length,1);
  assert.equal(keptReopen.conversation.find(item=>item.trace?.preview?.id===preview.preview.id).trace.preview.status,'kept');
  const blockedHandoff=await fetch(`${base}/${kept.projectId}/handoff`,{headers:{authorization:`Bearer ${kept.accessToken}`}});
  assert.equal(blockedHandoff.status,409);assert.equal((await blockedHandoff.json()).error,'WORKING_DRAFT_SAVE_REQUIRED');
  const blockedDiscussion=await post(kept,'/discussions',{requestId:'draft-discussion'});
  assert.equal(blockedDiscussion.status,409);assert.equal((await blockedDiscussion.json()).error,'WORKING_DRAFT_SAVE_REQUIRED');
  assert.equal(discussionWrites,0);
  let draftStore=sceneStoreForVersion(deserializeVersionHistory(keptReopen.versionHistory));
  for(const command of keptReopen.workingDraft.commands)draftStore=dispatchSceneCommand(draftStore,command);
  assert.equal(serializeScene(draftStore.currentScene),serializeScene(preview.scene));
  const staleSave=await post(kept,'/save',{requestId:'save-without-kept-preview',expectedRevision:2,versionHistory:kept.versionHistory});
  assert.equal(staleSave.status,409);assert.equal((await staleSave.json()).error,'WORKING_DRAFT_HISTORY_CONFLICT');
  const nextHistory=saveSceneVersion(deserializeVersionHistory(keptReopen.versionHistory),draftStore,{source:'agent-provider'});
  const saveResponse=await post(kept,'/save',{requestId:'save-kept-preview',expectedRevision:2,versionHistory:serializeVersionHistory(nextHistory)});
  assert.equal(saveResponse.status,200);assert.equal((await saveResponse.json()).revision,3);
  const savedReopen=store.read(kept.projectId,kept.accessToken);
  assert.equal(savedReopen.workingDraft,null);assert.equal(savedReopen.pendingPreview,null);
  assert.equal(serializeScene(sceneStoreForVersion(deserializeVersionHistory(savedReopen.versionHistory)).currentScene),serializeScene(preview.scene));
  assert.equal(savedReopen.conversation.find(item=>item.trace?.preview?.id===preview.preview.id).trace.preview.status,'saved');
  assert.equal(savedReopen.conversation.find(item=>item.trace?.preview?.id===preview.preview.id).trace.preview.savedVersionId,nextHistory.currentVersionId);
  const handoffResponse=await fetch(`${base}/${kept.projectId}/handoff`,{headers:{authorization:`Bearer ${kept.accessToken}`}});
  assert.equal(handoffResponse.status,200);
  const handoff=await handoffResponse.json();
  assert.equal(handoff.traces.find(item=>item.preview?.id===preview.preview.id).preview.status,'saved');
});
