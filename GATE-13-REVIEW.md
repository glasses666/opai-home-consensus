...
状态：已实现；拖拽持续命中、复合资产与编辑俯视焦点回归问题已修复，等待用户验收。
...
- 可编辑家具使用房间俯视时，镜头现在沿用已有对象包围盒跟随逻辑，将相机与 look-at 目标一起平移到选中家具中心；入口、主功能面和对象专属镜头不受影响。
- 后台页面复验：直接打开客餐厅沙发编辑 URL 后，沙发位于俯视画面中心，选中框与变换手柄围绕沙发本体。
- 验证：`npm test` 143 / 143，`npm run eval:agent` 28 / 28，`npm run build` 通过（仅保留既有大 chunk 警告）。
