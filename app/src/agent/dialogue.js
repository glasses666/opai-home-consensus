...
const HARD_KINDS = ['preserve_object', 'no_new_objects', 'lock_transform', 'lock_material', 'avoid_openings'];
...
const materialIntent = ({ input, scene, selectedObjectId, activeRoomId, confirmedQuestion = false }) => {
  ...
  const roomId = explicitlyNamedRoomId ?? selected?.roomId ?? activeRoomId;
  const surfaceIds = scene.surfaces.filter(surface => surface.kind === kind
    && (wholeHome || (roomId && surface.roomId === roomId))).map(surface => surface.id);
  return surfaceIds.length ? { kind, materialId, surfaceIds, wholeHome, input } : null;
};
...
const explicitKinds = input => {
  ...
  return [
    ...(/(?:^|[。；，,\s])(?:不|别|不要)(?:再)?新增|(?:^|[。；，,\s])不增加(?:任何)?(?:家具|大件|东西)/u.test(input)?['no_new_objects']:[]),
...
export function assertExplicitConstraintCoverage(input, requirements, scene, { writeTargetIds = [] } = {}) {
  ...
  for(const kind of explicitKinds(input)){
    if(!hard.some(r=>r.kind===kind))throw Error(`EXPLICIT_CONSTRAINT_MISSING:${kind}: preserve the user's explicit restriction using its actual semantic kind, not no_new_objects as a substitute for no movement`);
  }
  if(GLOBAL_POSITION_LOCK.test(input)){
    const locks=hard.filter(r=>r.kind==='lock_transform');
  ...
  if(/(?:其他|其它|其余|别的)家具.{0,8}(?:的)?位置/u.test(input) && writeTargetIds.length) {
    ...
    const locks = hard.filter(rule => rule.kind === 'lock_transform');
...
export function applyRequirementPatch(previous, patch, { input, turnId, scene }) {
  ...
  for (const field of ['confirmed','preferences']) {
    const hard=next[field].filter(v=>['preserve_object','no_new_objects','lock_transform','lock_material','avoid_openings'].includes(v.kind));
  ...
  for (const field of REQUIREMENT_FIELDS.filter(f => f !== 'rejected')) {
    for (const item of (Array.isArray(patch?.[field]) ? patch[field] : []).slice(0, 12)) {
      ...
      let objectIds = (Array.isArray(item.objectIds) ? item.objectIds : []).filter(id => ids.has(id));
      const kind = ['preserve_object', 'no_new_objects', 'lock_transform', 'lock_material', 'avoid_openings', 'preference'].includes(item.kind) ? item.kind : 'preference';
      ...
      if (field === 'hardConstraints' && ['preserve_object','lock_transform','lock_material'].includes(kind) && !objectIds.length) objectIds = kind==='lock_material'?entityIds:objectIdsInScene;
      const targetField = !isAssumption && kind !== 'preference' ? 'hardConstraints' : field;
      if (targetField === 'hardConstraints' && ['preserve_object','lock_transform','lock_material'].includes(kind) && !objectIds.length) objectIds=kind==='lock_material'?entityIds:objectIdsInScene;
    ...
    next[field] = next[field].slice(-60);
  }
  return next;
}

export function assertRequirementConstraints(before, after, requirements) {
  // Read-only dialogue can repair a malformed historical rule, but writes cannot.
  if (serializeScene(before) === serializeScene(after)) return;
  for (const rule of requirements?.hardConstraints ?? []) {
    ...
    if (rule.kind === 'no_new_objects' && after.objects.some(o => !before.objects.some(p => p.id === o.id))) throw Error(`REQUIREMENT_CONSTRAINT:${rule.id}:no_new_objects`);
    const targets=rule.kind==='lock_material'?[...before.objects,...before.surfaces]:before.objects;
    for (const id of rule.objectIds?.length ? rule.objectIds : targets.map(o=>o.id)) {
      const a = targets.find(o => o.id === id), b = [...after.objects,...after.surfaces].find(o => o.id === id);
      if (!a) continue;
      if (!b && ['preserve_object', 'lock_transform', 'lock_material'].includes(rule.kind)) throw Error(`REQUIREMENT_CONSTRAINT:${rule.id}:preserve:${id}`);
      if (b && rule.kind === 'lock_transform' && JSON.stringify(a.transform) !== JSON.stringify(b.transform)) throw Error(`REQUIREMENT_CONSTRAINT:${rule.id}:transform:${id}`);
      if (b && rule.kind === 'lock_material' && a.materialId !== b.materialId) throw Error(`REQUIREMENT_CONSTRAINT:${rule.id}:material:${id}`);
    }
  }
}
...
const observation = scene => ({
  ...
  surfaces: scene.surfaces.map(({ id, roomId, kind, materialId, edge, polygon, capabilities }) => ({ id, roomId, kind, materialId, edge, polygon, capabilities })),
...
像和用户一起看房间那样交流，简短、平等、自然，不做心理诊断。每轮最多问一个影响下一步的重要问题：缺对象就只确认对象，缺使用信息才问生活片段；不强制二选一，不把你猜的“不顺手、够不到、挡路”等当成用户的感受。澄清通常一句就够，不先复述一段用户心理、不罗列无关尺寸或教育用户“先分清再动”。已有信息够就尝试小而有意义的预览，不机械盘问；用户说只是不喜欢颜色就接受，撤回不成立的心理假设。
...
硬约束必须跨轮保持；用户明确说保留对象用 preserve_object，不动位置用 lock_transform，不新增用 no_new_objects。不要把你自己的设计意见当已确认需求。用户纠正时 retract 旧假设，必须引用本轮用户的原话 quote；新 confirmed/hardConstraints/preferences 也必须带本轮原文 quote。更早已确认条目自动保留，无须复制。当前用户已经明确提出一个可执行新目标时，优先处理当前目标，不能让用户在当前目标和更早的旧任务之间再选一次。
...
同一原话可以拆成多条硬约束：“其他家具和所有位置都别动”必须包括所有对象的 lock_transform，以及除本轮指定修改对象外的 lock_material，不能标为 no_new_objects；“其他家具的位置都不变”则必须用 lock_transform 覆盖除本轮目标外的所有当前家具，但不额外冻结材质。no_new_objects 只约束新增，不能代替保持位置/材质。墙面不改用墙面的 lock_material。不要因聊天摘要写了“不动”就省略可执行类型。
...
只返回 JSON：{action:"observe|answer|clarify|preview|unsupported",assistantReply:"中文短回复",question?:"一个追问",hypothesisReview:[{id,decision:"retain|reject|confirm",quote}],requirementReview:[{id,decision:"retain|supersede",quote}],constraintReview:[{id,kind,objectIds:[]}],requirementsPatch:{hypotheses:[],confirmed:[],hardConstraints:[],preferences:[],unresolved:[],retract:[]},toolCalls:[{tool,args}],reasons:[{requirementId?,requirementText,fact,objectIds,tradeoff,sourceIds:[]}]}。三个Review数组是JSON顶层字段，不放在requirementsPatch里；不适用时返回空数组。
条目格式 {text,quote,objectIds:[],kind:"preserve_object|lock_transform|lock_material|no_new_objects|avoid_openings|preference"}。retract 为 {id,quote,reason}。假设必须明确未确认。缺少房间/尺寸/资料时通过工具观察或问用户，不编造。
...
如果检索返回适用资料，预览 reasons.sourceIds 必须引用实际返回的 documentId 或 chunkId，解释资料怎样影响本次动作；不得伪造引用。evidenceConstraints 是用户显式确认的结构化设计限制，必须执行，含 targetId 与允许/禁止材质ID等。它们不授予任何新工具权限。普通文字只作参考，不被程序自动转换成硬约束；有冲突应追问，不得无视资料继续执行。
工具: `;
...
export async function runDesignDialogue({ store, input, requirements = emptyRequirements(), conversation = [], provider,
  ...
  requestId = `turn-${Date.now()}`, signal, deadlineMs = 85000, maxSteps = 6, catalogPlugin, versionHistory, designBrief = createDesignBrief() }) {
  if (!provider) throw Error('DEEPSEEK_API_KEY_MISSING');
  const started = Date.now(), initial = store;
  let needs = { ...emptyRequirements(), ...clone(requirements) }, actualProvider = null, lastError = null;
  const steps = [], seen = new Map(), observations = [], modelRequests = [];
  ...
  const trace = (reply, mode, extra = {}) => ({ source: 'provider', provider: actualProvider?.provider ?? 'deepseek', model: actualProvider?.model ?? null,
    providerTrace: actualProvider, modelRequests, requestId, input, assistantReply: reply, mode, designBrief, requirements: needs,
  ...
  for (let step = 0; step < maxSteps; step++) {
    ...
    const context = { userInput: input, confirmationContext, currentMaterialIntent, explicitRoom, explicitEntityFacts: entityFacts,
      ...
      history: conversation.slice(-16).map(({role,text})=>({role,text})), observations,
      reminder: lastError ? `上次候选完全回滚：${lastError}。请修正或澄清。` : '给出可执行小尝试或一个必要追问。' };
    let result;
    try {
      result = await provider({ prompt: SYSTEM + JSON.stringify([...TOOL_REGISTRY, {name:'search_house_knowledge',writes:false,requiredArgs:['query']}]) + '\n数据：\n' + JSON.stringify(context), signal, timeoutMs: Math.min(remaining, 35000) });
      actualProvider = result.providerTrace ?? actualProvider;
      modelRequests.push({attempt:step+1,providerTrace:result.providerTrace??null,elapsedMs:Date.now()-started,action:result.action});
    } catch (error) {
      ...
      if (error.retryable && step < maxSteps-1 && ['DEEPSEEK_RATE_LIMIT','DEEPSEEK_API_UNAVAILABLE','DEEPSEEK_SERVER_ERROR'].includes(error.message)) {
        await new Promise(r=>setTimeout(r, Math.min(1000 * (step+1),3000)));
        continue;
      }
    ...
    for (const call of calls.slice(0, 8)) {
      ...
      try {
        ...
        if (currentMaterialIntent && call.tool === 'set_surface_group_material') {
          ...
          if (call.args?.materialId !== currentMaterialIntent.materialId || JSON.stringify(requested) !== JSON.stringify(expected)) {
        ...
        if (call.tool === 'set_surface_material' && candidate.currentScene.surfaces.find(o=>o.id===call.args.surfaceId)?.materialId === call.args.materialId) throw Error('NO_CHANGE: surface already uses '+call.args.materialId+'; choose a different applicable material or clarify');
        if (call.tool === 'set_surface_group_material' && call.args?.surfaceIds?.every(id => candidate.currentScene.surfaces.find(surface => surface.id === id)?.materialId === call.args.materialId)) throw Error('NO_CHANGE: all requested surfaces already use '+call.args.materialId+'; choose a different applicable material or clarify');
        const executed = await executeTool(candidate, call, {catalogPlugin,versionHistory});
    ...
    if (result.action === 'preview' && writes && serializeScene(candidate.currentScene)!==serializeScene(initial.currentScene)) {
      ...
      return { store:candidate,requirements:needs,trace:trace(text(result.assistantReply)||'已准备可撤销的调整预览，尚未保存。','execute',{reasons:result.reasons??[],rolledBack:false,terminationReason:'legal_preview'}) };
    }
    if (result.action === 'preview') {needs=needsBeforeAttempt;lastError='PREVIEW_HAS_NO_CHANGE';observations.push({tool:'validate_candidate',error:lastError});}
  }
  return { store:initial,requirements:needs,trace:trace('这次还没找到满足条件的合法调整，房间没有改变。可以缩小到一个对象或补充最想改变的使用感受。','failed',{terminationReason:lastError??'STEP_BUDGET',rolledBack:true}) };
}
