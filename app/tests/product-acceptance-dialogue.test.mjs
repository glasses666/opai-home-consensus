...
test('observed relative placement rejects guessed coordinates and permits deterministic repair', async () => {
  ...
  const prompts = [];
  const result = await runDesignDialogue({reviewProvider:approveReview, store: initial, input, requestId: 'relative-repair', provider: async ({prompt}) => {
    prompts.push(prompt);
...
test('model scene observation includes canonical axes and bounds rather than inferred screen directions', async () => {
  ...
  let observed;
  await runDesignDialogue({reviewProvider:approveReview, store: createSceneStore(scene), input: '腾一点活动地方', requestId: 'axes-observation',
...
test('a held-out correction retracts the earlier hypothesis and records only quoted user intent', async () => {
  ...
  const first = await runDesignDialogue({reviewProvider:approveReview,
  ...
  const second = await runDesignDialogue({reviewProvider:approveReview,
    store: first.store,
...
test('reconfirming an unchanged requirement keeps original provenance and permits a new attempt', async () => {
  ...
  const result=await runDesignDialogue({reviewProvider:approveReview,store:createSceneStore(scene),input,requirements:prior,requestId:'retry-same',provider:async()=>({
...
test('natural master-bedroom atmosphere request produces a constrained multi-object preview', async () => {
  ...
  const result=await runDesignDialogue({reviewProvider:approveReview,store:initial,input,activeRoomId:'room-primary-bedroom',requestId:'master-atmosphere',provider:async()=>({
    ...
    requirementsPatch:{...blankPatch(),
      ...
      hardConstraints:[
        ...
        {text:'不新增大件',quote:'别加大件',kind:'no_new_large_objects',objectIds:[]},
      ],
  ...
  assert.deepEqual(result.requirements.hardConstraints.map(item=>item.kind).sort(),['no_new_large_objects','preserve_object']);
});
...
test('a newly proposed hard constraint with a preference kind is rejected and repaired on the next model attempt', async () => {
  ...
  assert.equal(result.trace.modelRequests.filter(request=>request.purpose==='plan').length, 2);
  assert.ok(result.trace.modelRequests.some(request=>request.purpose==='review_proposal'));
...
test('no-new-large-object keeps the user scope narrow: a small lamp is allowed but a large addition is rejected', () => {
  const before = createDemoScene();
  const requirements = {
    ...emptyRequirements(),
    hardConstraints: [{ id: 'no-large', kind: 'no_new_large_objects', objectIds: [] }],
  };
  const withLamp = structuredClone(before);
  withLamp.objects.push({
    ...structuredClone(withLamp.objects[0]),
    id: 'object-small-lamp',
    category: 'lighting',
    dimensions: { width: 260, height: 480, depth: 260 },
  });
  assert.doesNotThrow(() => assertRequirementConstraints(before, withLamp, requirements));

  const withWardrobe = structuredClone(before);
  withWardrobe.objects.push({
    ...structuredClone(withWardrobe.objects[0]),
    id: 'object-new-wardrobe',
    category: 'storage',
    dimensions: { width: 1600, height: 2200, depth: 600 },
  });
  assert.throws(() => assertRequirementConstraints(before, withWardrobe, requirements),
    /REQUIREMENT_CONSTRAINT:no-large:no_new_large_objects/);
});
...
test('an illegal first candidate is atomically rejected and is not reported as a completed final tool', async () => {
  ...
  const requirements = {
    ...
    hardConstraints: [
      ...
      { id: 'no-large', text: '不新增大件', quote: '不新增大件', objectIds: [], kind: 'no_new_large_objects', sourceTurnId: 'prior', status: 'user_stated' },
    ],
  };
  let calls = 0;
...
test('provider failure returns a failed trace and preserves the exact scene', async () => {
  ...
  await assert.rejects(
    runDesignDialogue({reviewProvider:approveReview,
      store: initial,
      input: '想高级一点但说上来。',
...
test('outcome HTTP replay is idempotent and a failed Feishu sync stays recoverable without losing the linked version', async (t) => {
  ...
  const server = createAppServer({ experienceHandler: createExperienceRoutes({reviewProvider:approveReview, store, family, provider: async () => providerResult({ action: 'unsupported', assistantReply: 'unused' }) }) });
