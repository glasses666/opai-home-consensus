...
export function createExperienceRoutes({store,knowledge,family,provider=callDesignDeepSeek,now=()=>Date.now()}={}){
  const rate=new Map();
  ...
  return async(req,res)=>{
    ...
    try{
      ...
      if(action==='turn'&&req.method==='POST'){
        ...
        const result=await store.transact(projectId,token,{...body,operation:'turn',payload:{input:body.input,scene:body.scene,versionHistory:body.versionHistory,selectedObjectId:body.selectedObjectId,spaceId:body.spaceId}},async s=>{
          ...
          const reply=await runDesignDialogue({store:working,input:body.input,requirements:s.requirements,conversation:s.conversation,
            provider,knowledge,projectId,houseId:s.houseId,selectedObjectId:body.selectedObjectId,activeRoomId:body.spaceId,
          ...
          s.requirements=reply.requirements;
          ...
          return {requirements:s.requirements,evidenceConstraints:constraintsFor(s),conversation:s.conversation,scene:reply.store.currentScene,commands:reply.store.commands.slice(working.cursor),trace:reply.trace};
      ...
      if(action==='handoff'&&req.method==='GET'){
        ...
        send(res,200,{projectId,versionId:h.currentVersionId,generatedAt:new Date().toISOString(),requirements:project.requirements,
          ...
          traces:project.conversation.filter(v=>v.trace).map(v=>({requestId:v.trace.requestId,provider:v.trace.provider,model:v.trace.model,mode:v.trace.mode,reasons:v.trace.reasons,retrieval:v.trace.retrieval,terminationReason:v.trace.terminationReason})),
    ...
    }catch(e){const code=safeError(e);send(res,/ACCESS_DENIED/.test(code)?403:/NOT_FOUND/.test(code)?404:/CONFLICT|STALE|BUSY/.test(code)?409:/RATE_LIMIT/.test(code)?429:/DEEPSEEK|DEADLINE|UNAVAILABLE/.test(code)?503:400,{error:code,...(e.trace?{trace:e.trace}:{}),message:'本次操作未完成；已保存的方案保持不变。'});}
    return true;
  };
}
