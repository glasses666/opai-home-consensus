"""Render V14 including every near-contact rebound frame."""
...
folder=OUT/'frames-v14';folder.mkdir(exist_ok=True)
cache={};rendered=0
for f in range(1,97):
    s.frame_set(f)
...
(OUT/'render-v14-report.json').write_text(json.dumps({'frames':96,'unique_states_rendered':rendered,'identical_static_states_reused':96-rendered,'device':'METAL','resolution':[1920,1080],'samples':64},indent=2))
