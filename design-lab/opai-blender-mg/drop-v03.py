...
for obj in [o for o in scene.objects if o.parent and o.parent.name=='OPPEIN_ASSEMBLY']:
    ...
    if '_stroke_' in obj.name:
        ...
        release=(11 if obj['drop_layer']=='upper' else 1)+col*2
    else:release={'Orange bracket 2':1,'Orange bracket 0':11,'Orange bracket 1':21}[obj.name]
...
for r in records:
    if '_stroke_1' in r['name']:
        lower=next(p for p in records if p['name']==r['name'].replace('_stroke_1','_stroke_0'))
        assert r['impact']>lower['settled']
        assert r['impact']-lower['settled']<5
scene.frame_set(75)
scene.render.filepath=str(OUT/'v03-assembled.png')
bpy.ops.wm.save_as_mainfile(filepath=str(OUT/'oppein-mg-drop-v03.blend'))
bpy.ops.render.render(write_still=True)
scene.frame_set(35);scene.render.filepath=str(OUT/'v03-dropping.png');bpy.ops.render.render(write_still=True)
(OUT/'drop-v03-manifest.json').write_text(json.dumps({'gravity':G,'height':HEIGHT,'restitution':RESTITUTION,'bounce_height':HEIGHT*RESTITUTION**2,'fps':FPS,'frames':84,'pieces':records,'verification':'unit scale; vertical-only; no seat undershoot; increasing fall speed; each upper impacts after its own lower settles with less than 5-frame gap','limitation':'Analytic choreography, not rigid-body contact solver or full mesh intersection proof'},indent=2))
print('DROP_V03_CHECKS_PASSED')
