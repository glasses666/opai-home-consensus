...
原布局把矮画桌、座椅和摇摇兔放在开放的玩耍区，小床横靠后墙。变化时，座椅与摇摇兔先向上飞离默认画面，画桌让位；同一张床平移、转向，贴靠左墙；玩具柜和画桌移到后墙，组成学习角。小件回到地面，摇摇兔仍保留在房间里。没有用替换一批家具来表现成长。
...
0–1.00 s：原布局；1.00–7.80 s：错峰变化；7.80–10.20 s：调整后布局；10.20–17.00 s：沿避让路径复位；17.00–18.00 s：原布局。
...
| 独立父对象 | 内容 |
| --- | --- |
| `Furniture_Daybed` | 云朵小床 |
| `Furniture_Desk` | 画桌 / 书桌 |
| `Furniture_Cubby` | 玩具 / 书本柜 |
| `Furniture_Chair` | 小座椅 |
| `Furniture_Rocker` | 摇摇兔 |
...
models/roomlet-growing-loop.glb         18 秒动画
models/roomlet-growing-original.glb     原布局静态模型
models/roomlet-growing-modified.glb     调整后静态模型
...
参见 **`docs/QA.md`** 和对应 JSON。当前模型有 103 个命名场景节点（GLB 另含 1 个默认相机节点）、5 组家具、17 种材质、3 张内嵌纹理；实例化后约 49,180 个三角形。循环 GLB 大小约 1.51 MiB。
