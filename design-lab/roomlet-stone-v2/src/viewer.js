...
(() => {
  ...
  class RoomletViewer {
    constructor(canvas,data,options={}){
      ...
      this.duration=data.motion.duration;this.times=decode(data.motion.times);this.time=0;this.speed=1;
    ...
    applyTime(seconds){
      ...
      this.onTime?.(this.time,this.phase());
    }
    tick(now){
    ...
    phase(){
      const reverse=this.time>this.duration/2,t=reverse?this.duration-this.time:this.time;
      let label=this.data.motion.phases[0].label;
      for(const p of this.data.motion.phases)if(t>=p.time)label=p.label;
      if(reverse){
        if(t<.85)label='原布局 · 相聚';
        else if(t<2.08)label='小件落回';
        else if(t<2.78)label='餐桌复位';
        else if(t<5.0)label='沙发回转';
        else if(t<5.95)label='桌子让位';
        else if(t<7.1)label='再次升空';
        else label='修改布局 · 舒展';
      }
      return {label,reverse,layout:t<.85?'original':t>=7.1?'modified':'transition',progress:this.time/this.duration};
    }
