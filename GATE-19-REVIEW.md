...
- 当前对象只保留三个不同层级的入口：`让 Agent 调整`、`快速微调`、`高级编辑`。
- Pascal 保留为同一 canonical scene 的 2D / 3D 主画布；完整工具栏只在用户明确进入高级编辑后展开。
- 退出高级编辑只切换宿主层级，不重建 scene，也不改 room / view / select / version。

## 产品精神复核

- Agent-first：通过。首屏主要任务是表达需求并让 Agent 推进设计。
- 用户编辑为辅：通过。快速微调与高级编辑都有入口，但视觉权重低于 Agent 主动作。
